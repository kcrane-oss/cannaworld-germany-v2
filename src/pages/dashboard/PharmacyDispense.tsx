...
export default function PharmacyDispense() {
  ...
  async function handleSubmit() {
    ...
    if (!form.btm_rx_no || form.btm_rx_no.length < 4) {
      setFeedback({ kind: "err", text: "BtM-Rezept-Nr fehlt" });
      return;
    }
    if (!form.contraindication_check || !form.patient_counseling) {
      setFeedback({ kind: "err", text: "Beide Apotheker-Pflichten müssen dokumentiert sein" });
      return;
    }
    ...
    try {
      ...
      setFeedback({ kind: "ok", text: `Abgabe protokolliert. Reststand: ${res.remaining_stock_units ?? "?"} Einheiten.` });
      ...
      <section className="grid gap-6 md:grid-cols-[1fr_1.4fr]">
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-sm font-bold uppercase tracking-wider text-white/70">Verfügbare Chargen</h2>
            {loading && <Loader2 className="h-4 w-4 animate-spin text-white/40" />}
          </div>
          {!loading && batches.length === 0 && <p className="text-sm text-white/50">Keine abgabebereiten Chargen.</p>}
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
          {!selected ? (
            <div className="flex h-full flex-col items-center justify-center text-center text-white/45">
              ...
              <p className="text-sm">Charge zur Abgabe wählen.</p>
            </div>
          ) : (
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-white">Abgabe: {selected.batch_number ?? selected.id.slice(0, 8)}</h2>

              <div className="grid gap-3 md:grid-cols-2">
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Apotheke-Lizenz</div>
                ...
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Apothekerkammer-ID</div>
                ...
                <label className="text-xs md:col-span-2">
                  <div className="mb-1 font-bold uppercase tracking-wider text-violet-200">BtM-Rezept-Nr (wird sofort gehashed)</div>
                ...
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Arzt-LANR (optional)</div>
                ...
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">KV-Nr (optional, pseudonymisiert)</div>
                ...
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Abgegebene Einheiten</div>
                ...
                <label className="text-xs">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Abgegebenes Gewicht (g)</div>
                ...
                <label className="text-xs md:col-span-2">
                  <div className="mb-1 font-bold uppercase tracking-wider text-white/55">Securpharm Decommission-ID</div>
              ...
              <div className="rounded-2xl border border-amber-300/20 bg-amber-400/[0.04] p-3">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <input placeholder="SCA-Proof" className="rounded-xl border border-white/15 bg-black/20 px-3 py-2 text-sm" value={form.sca_proof} onChange={(e) => setForm((f) => ({ ...f, sca_proof: e.target.value }))} />
                </div>
              </div>
              ...
              <button type="button" disabled={busy} onClick={handleSubmit}
                ...
                {busy ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : "Abgabe signieren"}
              </button>
            </div>
          )}
        </div>
